...
// Method which handles the clock up top as well as the timer
...
// Method which stops and resets the timer.
...
// BACK CLICK HANDLER
static void timer_back_click_handler (ClickRecognizerRef recognize, void *contect) {
   if (timer_finished_state == true) { // If timer is finished (alarm going off), reset
    stop_timer();
   } else {
...
void init_timer_window(void) {
  ...
  s_res_15_plus_button = gbitmap_create_with_resource(RESOURCE_ID_15_plus);
  s_res_play_button = gbitmap_create_with_resource(RESOURCE_ID_play);
  s_res_15_minus_button = gbitmap_create_with_resource(RESOURCE_ID_15_minus);
  s_res_pause_button = gbitmap_create_with_resource(RESOURCE_ID_pause);
  s_res_reset_button = gbitmap_create_with_resource(RESOURCE_ID_reset);
  s_res_play_reset_button = gbitmap_create_with_resource(RESOURCE_ID_play_reset);
